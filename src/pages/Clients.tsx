...
interface Client {
  ...
  ifu: string | null; rccm: string | null; sector: string | null
  num_employeur: string | null; nif: string | null; logo_url: string | null
  bp: string | null; entite_name: string | null
}
...
export default function Clients() {
  ...
  const openCreate = () => {
    setEditing(null); setForm(EMPTY_FORM); setLogoFile(null); setLogoPreview(null); setShowForm(true)
  }
  ...
  const uploadLogo = async (clientId: string): Promise<string | null> => {
    ...
    const { error } = await supabase.storage.from('client-logos').upload(path, logoFile, { upsert: true, contentType: logoFile.type })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) {
      ...
      await supabase.from('clients').update({ ...form, logo_url }).eq('id', editing.id)
      toast('Client mis à jour', 'success')
    } else {
      const { data: newClient } = await supabase.from('clients').insert({ ...form, organization_id: org.id }).select().single()
      if (newClient && logoFile) {
        const logo_url = await uploadLogo(newClient.id)
        if (logo_url) await supabase.from('clients').update({ logo_url }).eq('id', newClient.id)
      }
      toast('Client créé', 'success')
    }
    setUploadingLogo(false); setShowForm(false); fetchClients()
  }
  ...
  return (
    <div className="space-y-6 page-enter">
      ...
      <ConfirmModal open={!!deleting} title="Supprimer ce client" message={`Supprimer "${deleting?.name}" et toutes ses données ? Action irréversible.`} confirmLabel="Supprimer" danger onConfirm={handleDelete} onCancel={() => setDeleting(null)} />

      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Clients</h1>
          <p className="text-slate-500 mt-1">{clients.length} entreprise{clients.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} className="input pl-10" placeholder="Rechercher..." />
      </div>
            ...
            <div key={client.id} className="card p-5 hover:shadow-card-hover transition-all duration-300 group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  {client.logo_url
                    ? <img src={client.logo_url} alt="logo" className="w-10 h-10 object-contain rounded-lg border border-slate-100" />
                    : <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-100 to-accent-100 flex items-center justify-center"><Building2 className="w-5 h-5 text-primary-600" /></div>
                  }
                  <div>
        ...
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal max-w-2xl" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              {/* Logo upload */}
              <div>
                <label className="label">Logo de l'entreprise (affiché sur les bulletins)</label>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">PNG, JPG — recommandé 200×200px</p>
                  </div>
                </div>
              </div>

              <div><label className="label">Nom commercial *</label><input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="input" placeholder="SARL Exemple Togo" /></div>
              <div><label className="label">Entité / Raison sociale complète</label><input value={form.entite_name} onChange={e => setForm({...form, entite_name: e.target.value})} className="input" placeholder="Deutschen Volkshochschul-Verbandes e.v" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">N° Employeur CNSS</label><input value={form.num_employeur} onChange={e => setForm({...form, num_employeur: e.target.value})} className="input" placeholder="144472" /></div>
                <div><label className="label">NIF employeur</label><input value={form.nif} onChange={e => setForm({...form, nif: e.target.value})} className="input" placeholder="1001764361" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">Adresse / BP</label><input value={form.bp} onChange={e => setForm({...form, bp: e.target.value})} className="input" placeholder="05 BP 1189 LOME 05 TG" /></div>
                <div><label className="label">Téléphone</label><input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="input" placeholder="90252534/91383762" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">Adresse physique</label><input value={form.address} onChange={e => setForm({...form, address: e.target.value})} className="input" /></div>
              ...
              <div><label className="label">Secteur d'activité</label><input value={form.sector} onChange={e => setForm({...form, sector: e.target.value})} className="input" /></div>
