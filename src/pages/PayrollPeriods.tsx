...
export default function PayrollPeriods() {
  ...
  const fetchData = async () => {
    ...
    const { data } = await supabase.from('payroll_periods').select('*, clients(name)').in('client_id', clientIds).order('period_year', { ascending: false }).order('period_month', { ascending: false })
    setPeriods(data || [])
    setLoading(false)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (existing) { toast('Période déjà existante pour ce client et ce mois.', 'warning'); return }
    ...
    if (!error && data) { setShowForm(false); toast('Période créée', 'success'); navigate(`/payroll/${data.id}`) }
  }
  ...
  return (
    <div className="space-y-6 page-enter">
      ...
      <ConfirmModal
        open={!!closing} title="Clôturer la période"
        message="Cette période sera archivée. Aucune modification ne sera possible ensuite."
        confirmLabel="Clôturer" danger
        onConfirm={() => closing && handleClose(closing)}
        onCancel={() => setClosing(null)}
      />

      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Périodes de paie</h1>
          <p className="text-slate-500 mt-1">Gestion mensuelle par client</p>
        </div>
        <button onClick={() => { setForm({ ...form, client_id: clients[0]?.id || '' }); setShowForm(true) }}
          className="btn-primary" disabled={!clients.length}>
      ...
      {!clients.length ? (
        ...
        <div className="space-y-8">
          ...
          {openPeriods.length > 0 && (
            <div>
              <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                <Clock className="w-4 h-4 text-amber-500" /> Périodes ouvertes ({openPeriods.length})
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {openPeriods.map(p => <PeriodCard key={p.id} period={p} onClose={() => setClosing(p.id)} />)}
              </div>
            </div>
          )}
          ...
          {closedPeriods.length > 0 && (
            <div>
              <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Périodes clôturées ({closedPeriods.length})
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {closedPeriods.map(p => <PeriodCard key={p.id} period={p} />)}
              </div>
            </div>
          )}

          {!periods.length && (
            <div className="card p-12 text-center"><CalendarClock className="w-12 h-12 text-slate-200 mx-auto mb-3" /><p className="text-slate-500">Aucune période.</p></div>
          )}
        </div>
      )}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="label">Année *</label>
                  <input type="number" required value={form.period_year} onChange={e => setForm({...form, period_year: Number(e.target.value)})} className="input" />
                </div>
              </div>
    ...
    <div className={`card p-5 hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-300 ${isOpen ? 'border-l-4 border-l-amber-400' : 'border-l-4 border-l-emerald-400'}`}>
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-sm text-slate-500">{period.clients?.name}</p>
        </div>
      ...
      <div className="flex gap-2 mt-3">
        <Link to={`/payroll/${period.id}`} className="btn-secondary flex-1 text-sm justify-between">
          {isOpen ? 'Saisir variables' : 'Consulter'}
          <ChevronRight className="w-4 h-4" />
        </Link>
        {isOpen && onClose && (
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-red-50 hover:text-red-600 text-slate-400 transition-colors border border-slate-200" title="Clôturer">
