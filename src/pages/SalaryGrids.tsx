                ...
                <table className="w-full">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      ...
                      <th className="text-right px-4 py-3 text-xs font-semibold text-slate-600 uppercase">Taux horaire</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filtered.map((grid) => (
                      <tr key={grid.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm text-right text-slate-600">{formatXOF(grid.hourly_rate)}</td>
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-1">
                            ...
                            <button onClick={() => handleDelete(grid.id)} className="p-1.5 rounded-lg hover:bg-error-50 text-slate-500 hover:text-error-600"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div><label className="label">Taux horaire (FCFA)</label><input type="number" min="0" step="0.01" value={form.hourly_rate} onChange={(e) => setForm({ ...form, hourly_rate: Number(e.target.value) })} className="input" /></div>
